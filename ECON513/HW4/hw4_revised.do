...
forval i =1/8{
sum e if group == `i'
scalar avgegroup`i' = r(sum)/n`i'
replace ecluster = avgegroup`i' if group == `i'
}

gen etilde = e - ecluster
mkmat etilde, mat(etilde)

// solve the etilde, sigma^2_tilde, thus rho

mat variance = (etilde'*etilde)/(391-8-1)

scalar vartilde = variance[1,1]
